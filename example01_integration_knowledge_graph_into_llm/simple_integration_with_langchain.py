...
model = OllamaLLM(
    model=ollama_vars["model"],
    base_url=ollama_vars["host"]
...
def ask_astronomy_question(question):
    ...
    if info:
        ...
        template_eng = """Use the following pieces of context to answer the question at the end in german language. If you don't know the answer, just say that you don't know, don't try to make up an answer.\n\n{context}\n\nQuestion: {question}\nHelpful Answer:"""
        template_ger = """Beantworte die Frage am Ende in deutsch mit Hilfe der folgenden Informationen.\n\n{context}\n\nFrage: {question}\n"""
        prompt = ChatPromptTemplate.from_template(template_ger)
        chain = prompt | model

        result = chain.invoke({"question": question, "context": summary})
        return result
    else:
